...
// where ts.t_ur_id=ur.ur_id and ur.u_id=ui.u_id
// and ts.s_ur_id={$ur_id}
// order by ts.time_out is not null, ts.time_out desc;";

$session_history_query =
"select ui.fname, ui.lname, ts.time_in, ts.time_out, ts.t_ur_id, ts.c_id, crs.course_name
from USER_INFO ui, TUTOR_SESSION ts, USER_ROLES ur, COURSE crs
 where ts.t_ur_id=ur.ur_id and
	ur.u_id = ui.u_id and
...
$history_stmt->bind_result($fname,$lname,$time_in,$time_out,$t_ur_id, $course_id, $course_name);
