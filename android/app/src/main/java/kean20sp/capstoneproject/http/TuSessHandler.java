...
public class TuSessHandler extends HTTPConnectionHandler{
    private String host = null, filepath = null;

    public JSONArray tutor_sessions;

    public TuSessHandler(String host, String filepath){
        this.host = host;
        this.filepath = filepath;
    }
    public TuSessHandler(){
        host = "seesselm-project-page.com";
        filepath = "/Capstone/android/Tutor_Session_history.php";
    ...
    public String getTutorSessionHistory(String user_role_id){
    ...
    private String process(String response) {
        System.out.println("TuSessHandler "+response);
        try {
            System.out.println("asdfasdf: "+response);
            tutor_sessions = new JSONArray(response);
            return SUCCESS;
        }catch(Exception e){
            e.printStackTrace();
            return FAILURE;
