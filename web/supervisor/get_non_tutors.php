...
$d_id=$row['d_id'];
$non_tutor="Select ui.fname as fname,ui.lname as lname,ui.year_at_organization as year,ui.u_id as user_id from $server_database.USER_INFO ui where ui.u_id not in (Select u.u_id from $server_database.USER_ROLES u where u.r_id=$tutor and u.ur_id in (Select ur_id from $server_database.USER_ROLES_DEPARTMENT where d_id=$d_id))";
